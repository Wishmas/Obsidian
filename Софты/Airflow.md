...
![600](../Вложения/Airflow/Pasted%20image%2020251030224222.png)<br>
<br>
...
**~={red}Граф=~** — это структура, которая состоит из **узлов** (или вершин) и **связей** между ними, которые называются рёбрами.<br>
...
~={cyan}**Оператор**=~ в Airflow выполняет функцию запуска определённого кода.<br>
...
 В Airflow задачи (Task) выполняются в разных процессах или даже на разных машинах. Это означает, что нельзя напрямую передавать результат одной задачи другой в ходе выполнения.<br>
<br>
